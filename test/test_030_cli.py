...
def test_single_basic():
    ...
    with TemporaryDirectory() as tmp:
        ...
        ret = os.system(f"python -m ts2d -i {fp} -o {tmp} --model {get_test_model_single()}")
...
def test_single_full():
    ...
    with TemporaryDirectory() as tmp:
        fp = get_asset_path(f"{sample}.nrrd")
        ret = os.system(f"python -m ts2d -i {fp} -o {tmp} --visualize --save-all --model {get_test_model_multi()}")
        assert ret == 0, f"The CLI command failed with a non-zero exit code: {ret}"
...
def test_folder_basic():
    ...
    with TemporaryDirectory() as tmp:
        ...
        ret = os.system(f"python -m ts2d -i {dir_in} -o {dir_out} --model {get_test_model_single()}")
        assert ret == 0, f"The CLI command failed with a non-zero exit code: {ret}"
